...
(function (global) {
  ...
  // Two shipped example themes (a light scheme and a navy/gold corporate brand).
  registerTheme('light',     { bg:'#f4f6fb', panel:'#ffffff', panel2:'#eef2f8', line:'#c9d3e0', text:'#1a2230',
    muted:'#5b6b82', white:'#0b1119', hot:'#2563eb', bad:'#dc2626', idle:'#b7c2d4', dim:'#9fb0c6',
    accent:'#b45309', series:['#0e9f6e','#2563eb','#7c3aed'] });
  registerTheme('corporate', { bg:'#0b1e33', panel:'#12293f', panel2:'#0e2236', line:'#2a4a68', text:'#e6eefc',
    muted:'#8fa9c8', white:'#ffffff', hot:'#4f9dff', bad:'#ff6b6b', idle:'#3a5674', dim:'#46617f',
    accent:'#f4b942', series:['#4f9dff','#f4b942','#6ee7b7'] });
  ...
  class Box extends Component {
    constructor(id, spec){ super(id, spec);
      this.name=spec.name||id; this.sub=spec.sub||null; this.accent=spec.accent||Theme.line; }
    draw(g, env){
      Draw.box(g,this.x,this.y,this.w,this.h,{stroke:this.accent, glow:this.hovered?env.theme.hot:null});
      Draw.text(g,this.name,this.x+this.w/2,this.y+this.h/2+(this.sub?-2:5),{c:this.accent===Theme.line?env.theme.text:this.accent,size:13,align:'center',w:'600'});
  ...
  class Core extends Box {
    ...
    draw(g, env){
      Draw.box(g,this.x,this.y,this.w,this.h,{fill:Theme.panel2,stroke:Theme.line,r:9,glow:this.hovered?env.theme.hot:null});
      g.save(); g.strokeStyle=this.lit?this.accent:Theme.idle; g.globalAlpha=this.lit?0.9:0.5; g.lineWidth=1.4;
      Draw.roundRect(g,this.x+3,this.y+3,this.w-6,this.h-6,7); g.stroke(); g.restore();
      Draw.text(g,this.name,this.x+this.w/2,this.y+this.h/2+4,{c:this.hovered?env.theme.white:this.accent,align:'center',w:'600'});
    }
  }
  ...
  class RingBuffer extends Component {
    constructor(id, spec){ super(id, spec);
      this.slots=spec.slots||12; this.r=spec.r||16; this.color=spec.color||Theme.hot;
    ...
    draw(g, env){
      const cx=this._cx(), cy=this.y+this.h/2, col=this.lapping?env.theme.bad:this.color;
      ...
      if(this.label) Draw.text(g,this.label,cx+this.r+8,cy+3,{c:this.color,size:10.5});
    }
  }
  ...
  class Matrix extends Component {
    constructor(id, spec){ super(id, spec);
      ...
      this.colColors=spec.colColors||Theme.series; this.rowLabels=spec.rowLabels||[]; this.colLabels=spec.colLabels||[];
    ...
    draw(g, env){
      ...
      for(let j=0;j<this.cols;j++){ const c=this.cells[0][j].down?env.theme.bad:this.colColors[j];
      ...
      for(let i=0;i<this.rows;i++)for(let j=0;j<this.cols;j++){
        const x=this.colX(j), y=this.rowY(i), c=this.cells[i][j], f=c.fresh, col=this.colColors[j];
        const fill=c.down?'#2a1620':(f>0?Tween.mix('#0e1c28',col,f):'#0e1c28');
        Draw.box(g,x,y,this.cw,this.ch,{fill,stroke:'#16222f',glow:f>0.4?col:null,r:6});
  ...
  class Pipeline extends Component {
    constructor(id, spec){ super(id, spec);
      this.stages=spec.stages||['a','b']; this.nodeR=spec.nodeR||15; this.idle=!!spec.idle;
      this.vertical=!!spec.vertical; this.boxed=!!spec.boxed; this.pinned=!!spec.pinned;
      this.name=spec.name||null; this.accent=spec.accent||Theme.accent;
    ...
    draw(g, env){ const th=env.theme;
      if(this.boxed){ Draw.box(g,this.x,this.y,this.w,this.h,{fill:this.pinned?th.panel2:th.panel,stroke:this.accent,glow:this.hovered?th.hot:null,r:9});
        if(this.pinned){ g.save(); g.strokeStyle=this.accent; g.globalAlpha=0.5; g.lineWidth=1.2; Draw.roundRect(g,this.x+3,this.y+3,this.w-6,this.h-6,7); g.stroke(); g.restore(); }
        if(this.name) Draw.text(g,this.name,this.x+11,this.y+18,{c:this.hovered?th.white:this.accent,size:12,w:'600'}); }
      ...
      for(let k=0;k<this.stages.length;k++){ const nx=this.nodeX(k), ny=this.nodeY(k), lit=active && (env.now-this.pulseAt)*8>k;
        if(k<this.stages.length-1){ const nx2=this.nodeX(k+1), ny2=this.nodeY(k+1);
          ...
          g.stroke(); g.restore(); }
        const col=this.idle?th.idle:(k===0?this.accent:(k===this.stages.length-1?th.hot:th.text));
  ...
  class Zone extends Component {
    constructor(id, spec){ super(id, spec);
      ...
      this.accent=spec.accent||Theme.line; this.hoverable=false; }
    draw(g, env){ const th=env.theme;
      g.save(); g.fillStyle=this.tint; Draw.roundRect(g,this.x,this.y,this.w,this.h,12); g.fill();
      g.globalAlpha=0.55; g.strokeStyle=this.accent; g.lineWidth=1.2; g.setLineDash([5,4]);
  ...
  class Channel extends Component {
    constructor(id, spec){ super(id, spec); this.from=spec.from; this.to=spec.to;
      this.roadW=spec.roadW||16; this.color=spec.color||Theme.hot; this.label=spec.label||null;
      this.hoverable=spec.hoverable!==false; }
    ...
    draw(g, env){ const th=env.theme;
      ...
      this._path(g); g.strokeStyle='#0b131d'; g.lineWidth=this.roadW; g.stroke();                         // road bed
      this._path(g); g.strokeStyle=this.color; g.globalAlpha=this.hovered?0.30:0.16; g.lineWidth=this.roadW-3; g.stroke(); // surface
      this._path(g); g.strokeStyle=this.color; g.globalAlpha=0.8; g.lineWidth=1.4;                        // moving lane markings
  ...
  class Connector {
    ...
    draw(g, env){ const a=this._pt(this.from), b=this._pt(this.to);
      Draw.link(g,a[0],a[1],b[0],b[1],{c:this.opts.c||env.theme.line, alpha:this.opts.alpha==null?0.18:this.opts.alpha, dash:this.opts.dash});
  ...
  class FlowSystem {
    ...
    draw(g){ for(const p of this.packets){ const leg=p.route[p.i]; const st=leg.style||p.style;
      const q=this.pos(p); Draw.glow(g,q[0],q[1],st.r||3.4,st.color); } }
  }
  ...
  class Diagram {
    constructor(canvas, spec){ spec=spec||{};
      ...
      // paint a named non-default theme's background (so `theme corporate/light` visibly re-skins);
    ...
    env(){ return {now:this.now, dt:this._dt||0, theme:this.theme, flow:this.flow, diagram:this}; }
  ...
  const API = { Theme, Themes, registerTheme, resolveTheme, Rng, Tween, Draw, Component, Box, Core, RingBuffer, Matrix, Pipeline, Zone, Channel, Connector, FlowSystem, FlowRuntime, StateStore, Diagram };
...
(function (global) {
  ...
  function resolveZones(ir) {
    const tracks = autofillTracks(ir);
    const lanes = indexById(tracks.lanes), rails = indexById(tracks.rails), H = ir.height;
  ...
  function recorder() {
    return {
      components: [], zones: [], roads: [], connectors: [], flow: new DK.FlowSystem(), now: 0,
      add(c) { this.components.push(c); return c; },
      addZone(z) { this.zones.push(z); return z; },
      addRoad(r) { this.roads.push(r); return r; },
  ...
  function build(ir, target) {
    ...
    resolveZones(ir).forEach(z => diagram.addZone(new DK.Zone(z.id, z)));
    ...
    (ir.edges || []).forEach(e => {
      ...
      const handle = (e.kind === 'road')
        ? diagram.addRoad(new DK.Channel(e.id, Object.assign({}, e, { from, to })))
        : diagram.connector(from, to, e.style || {});
  ...
  function buildFlows(ir, ctx) {
    ...
    const styleFor = fl => () => ({ style: { color: fl.color || DK.Theme.hot, r: fl.r || 3.4 } });
    const legFor = fl => { const style = styleFor(fl);
      const leg = (a, b, penv) => {                             // a→b hop; pulses b + runs its actions on arrival
        const to = nodeOf(fl, tpl(b.node, penv));
        return { from: [nodeOf(fl, tpl(a.node, penv)), tpl(a.port, penv) || 'out'], to: [to, tpl(b.port, penv) || 'in'],
          dur: b.dur || 0.6, onArrive: p => { if (to.pulse) to.pulse(ctx.diagram.now); runActions(b.actions, penv, leg, style, p); } };
      }; return leg; };
    ...
    (ir.flows || []).forEach(fl => {
      ...
      const leg = legFor(fl);
      const pick = () => {                                       // weighted choose-one
        const opts = fork.options, total = opts.reduce((s, o) => s + (o.weight || 1), 0);
        let x = rand() * total;
      ...
      rt.source({
        ...
        fire: () => {
          ...
          if (fork.mode === 'pick') {
            ctx.diagram.flow.spawn(prefix.concat([leg(from, pick(), penv)]), style());
  ...
  const API = { build, recorder, register, KINDS, resolveLayout, resolveZones, buildFlows, dimDefaults: DIM_DEFAULTS,
    layoutDefaults: { pad: LAYOUT_PAD, gap: LAYOUT_GAP } };
  if (typeof module !== 'undefined' && module.exports) module.exports = API;
 ...
 *   palette <name> = <color> <color> ...        # enables palette refs, e.g. `name0`, `name1`
 *   zone  <id> "Label" x:.. y:.. w:.. h:..    # or infer the box: `lane:<id>`/`lanes:[..]` (+ `rail:`/`rails:[..]`)
 *   node  <id> <kind> [flag ...] [key:value ...]
 ...
 * Values coerce: 12 → number, #abc → colour, name3 → palette[name][3], a|b|c → array, "x" → string,
...
(function (global) {
  ...
  // Coerce a value token to number / colour / palette ref / array / string.
  //   [a, b, "c d"] → array (comma/space separated, quote-aware); each element coerces.
  //   "…"           → ALWAYS a literal string (quotes never make an array).
  //   #rgb/#rrggbb  → colour · name<digit> → palette ref · 12/-3.5 → number · else bareword.
  function coerce(tok, palettes) {
    const quoted = tok[0] === '"';
    ...
    if (!quoted && tok[0] === '[' && tok[tok.length - 1] === ']')  // bracket list
      return splitList(tok.slice(1, -1)).map(x => coerce(x, palettes));
    const s = unquote(tok);
    if (quoted) return s;                                          // a quoted literal is always a string
    if (/^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(s)) return s;    // colour
    const pal = /^([A-Za-z_]+)(\d+)$/.exec(s);                     // palette ref: series0, feed2, …
    if (pal && palettes[pal[1]]) {
      const arr = palettes[pal[1]], i = +pal[2];
      if (i >= arr.length)                                         // catch a typo'd index, don't leak a string
        throw new Error('flow: palette "' + pal[1] + '" has no index ' + i + ' (size ' + arr.length + ')');
      return arr[i];
    }
  ...
  // Fold "flag" and "key:value" tokens into an object.
  function applyKV(obj, toks, palettes) {
    for (const t of toks) {
      const c = t.indexOf(':');
      if (c > 0 && t[0] !== '"') obj[t.slice(0, c)] = coerce(t.slice(c + 1), palettes);
      else obj[unquote(t)] = true;                                 // bare token → boolean flag
    }
    return obj;
  }
  ...
  // parse), so a shared palette / node library merges into the diagram. Resolution:
  ...
  function parse(text, opts) {
    ...
    const ir = { zones: [], nodes: [], edges: [] }, palettes = {};
    expand(text).forEach(({ text: line, ln }) => {
      // Tokenize first, THEN drop a trailing comment — so `#5ef2a0` (a colour) survives while
      // `# note` (hash then space) and `#note` (hash then non-hex) are treated as comments.
      const toks = tokenize(line);
      const ci = toks.findIndex(tk => tk === '#' ||
        (tk[0] === '#' && !/^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(tk)));
      ...
      switch (kw) {
        ...
        case 'behavior': {   // behaviour section header; may carry `seed:N` inline (`behavior seed:42`)
          const rest = applyKV({}, t.slice(1), palettes);
        ...
        case 'every': {      // `every <rate>[s] per <v> in <list> [when <cond>]: <actions>` → ir.periodics
          ...
          let list;
          if (m[3][0] === '[') list = coerce(m[3], palettes);
        ...
        case 'palette': {
          const name = t[1], eq = t.indexOf('=');
          palettes[name] = t.slice(eq + 1);
          (ir.palette || (ir.palette = {}))[name] = palettes[name];
          break;
        }
        case 'zone': {
          const z = { id: t[1] }; let i = 2;
          if (t[i] && t[i][0] === '"') { z.label = unquote(t[i]); i++; }
          applyKV(z, t.slice(i), palettes); validateKeys(z, 'zone', ln); ir.zones.push(z);
          break;
        }
        case 'lane': {   // layout column: lane <id> x:.. w:..  (used by node lane:<id>)
          const L = { id: t[1] };
          applyKV(L, t.slice(2), palettes);
        ...
        case 'rail': {   // layout row: rail <id> <y>  or  rail <id> y:..  (used by node rail:<id>)
          ...
          applyKV(R, t.slice(i), palettes);
          (ir.rails || (ir.rails = [])).push(R);
          break;
        }
        case 'node': {
          const n = { id: t[1], kind: t[2] };
          applyKV(n, t.slice(3), palettes); validateKeys(n, n.kind, ln); ir.nodes.push(n);
          break;
        }
        case 'edge':
        case 'road': {
          ...
          const rest = applyKV({}, t.slice(ai + 2), palettes);
          if (rest.id) { e.id = rest.id; delete rest.id; }
          if (kw === 'road') { e.kind = 'road'; Object.assign(e, rest); }
          else e.style = rest;
          ir.edges.push(e);
          break;
        }
        case 'flow': {   // behaviour: flow <id> rate:.. [color:.. r:.. max:..] : <route>
          ...
          if (pickIdx >= 0) {
            applyKV(f, header.slice(0, pickIdx), palettes);
          ...
          if (rt.length) {
            ...
            if (!sep) {
              ...
              const opt = h => Object.assign(clean(h), { weight: h.weight != null ? h.weight : 1 });
              ...
              for (let s = 1; s < segs.length; s++) {
                ...
                const h = Object.assign(rn(seg[1]), { dur });   // ~dur node [@weight] [{actions}]
                let j = 2;
...
(function (global) {
  ...
  function mount(target, source, opts) {
    ...
    const diagram = isDiagramLike(target) ? target
      : new DK.Diagram(typeof target === 'string' ? document.querySelector(target) : target,
          Object.assign({ width: ir.width, height: ir.height, theme: ir.theme }, opts.diagram));
    ...
    return { ir, diagram, rt, byId: built.byId, edgesById: built.edgesById, dispose };
  }
  ...
  function boot(root, mountFn) {
    ...
    doc.querySelectorAll('script[type="text/flow"][data-flowdot]').forEach(el => {
      ...
      renderModeControls(doc, canvas, res);                           // zero-JS toggle per declared mode
      if ((el.dataset && el.dataset.controls != null) || (res.ir && res.ir.controls))
        renderTransportControls(doc, canvas, res);                    // zero-JS play/pause + reset + speed
      res.exportPNG = exportPNG(canvas, doc, res.ir && res.ir.title); // imperative: save the frame as PNG
      if (el.dataset && el.dataset.export != null) renderExportControl(doc, canvas, res.exportPNG);  // opt-in button
      // Opt-in live editing: `data-editable` makes the data-source panel editable; a debounced edit
      // disposes the current diagram and re-mounts the new source into the SAME canvas. (Inline error
      // display for a bad edit is wired in the next task.) Re-mount skips showSource so it doesn't
      // clobber what the user is typing.
      if (el.dataset && el.dataset.editable != null && el.dataset.source && doc.querySelector) {
        const panel = doc.querySelector(el.dataset.source);
        ...
        liveEdit(panel, text => {
          const t = (text || '').trim();
          try { validateSource(t, remountOpts.safe); }              // dry-run FIRST — never tear down on a bad edit
          catch (e) { showError(doc, panel, e.message); return; }   // keep the last good diagram, show the error
          try {
            if (current && typeof current.dispose === 'function') current.dispose();
            current = run(canvas, t, remountOpts);
            canvas.__flowdot = current;
            renderModeControls(doc, canvas, current);
            if ((el.dataset && el.dataset.controls != null) || (current.ir && current.ir.controls))
              renderTransportControls(doc, canvas, current);
            showError(doc, panel, null);                            // valid again → clear the error
  ...
  // demand). Keeps the previous diagram visible while the source is invalid. No-op without a DOM.
  ...
  const KEYWORDS = ['diagram', 'flow', 'seed', 'theme', 'palette', 'set', 'import', 'model',
    'lane', 'rail', 'zone', 'node', 'edge', 'road', 'flow', 'mode', 'every', 'on', 'call'];
  ...
  const API = { mount, boot, liveEdit, wirePickers, suggest, attachAutocomplete, exportPNG, applyA11y, prefersReducedMotion, renderTransportControls };
...
export const { Theme, Themes, registerTheme, resolveTheme, Rng, Tween, Draw, Component, Box, Core, RingBuffer, Matrix, Pipeline, Zone, Channel, Connector, FlowSystem, FlowRuntime, StateStore, Diagram } = __F;
export const SceneBuilder = __S;
export const Flow = __Z;
export const mount = __F.mount;
export const boot = __F.boot;
export default Object.assign({}, __F, { SceneBuilder: __S, Flow: __Z });
